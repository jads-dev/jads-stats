<template>
  <v-row>
    <v-col md="3" cols="6" ><v-select dense :items="order_bys" v-model="order_by" item-text="label" item-value="value" label="Order by" outlined></v-select></v-col>
    <v-col md="3" cols="6"><v-checkbox dense v-model="limit_results" label="Limit to 100 results (may cause lag if unchecked)"></v-checkbox></v-col>
    <v-col md="6" cols="0"></v-col>
    <v-col md="3" cols="6"><v-text-field dense label="Username Filter" outlined v-model="username_filter"></v-text-field></v-col>
    <v-col md="3" cols="6"><v-text-field dense label="Message Filter" outlined v-model="content_filter"></v-text-field></v-col>
    <v-col md="6" cols="0"></v-col>
    <v-progress-linear v-if="is_loading" indeterminate></v-progress-linear>
    <v-col cols="12">
      <span v-for="message in messages" v-bind:key="message.message_id" v-html="message.content_html">
      </span>
    </v-col>
  </v-row>
</template>
...
export default {
  data: () => ({
    ...
    content_filter: "",
    order_bys: [
      { label: "Reaction Count", value: "reaction_count desc" },
      { label: "Message Date", value: "message_id asc" },
    ],
    order_by: "reaction_count desc",
    limit_results: true,
  }),
  methods: {
    fetch_data: async function () {
      ...
      let qry = `
            select message_id, content_html
            from markov_2022
            where (user_name like '%${this.username_filter}%' or '${this.username_filter}' = '')
            and (message_content like '%${this.content_filter}%' or message_content = '')
            order by ${this.order_by}
        `;
      if (this.limit_results) qry += "\nLIMIT 100";

      let messages = await this.$dbworker.db.query(qry);

      this.messages = messages;
      this.is_loading = false;
    },
    fetch_file: async function (message_id) {
  ...
  watch: {
    username_filter: function (n, old) {
      this.fetch_data();
    },
    content_filter: function (n, old) {
      this.fetch_data();
    },
    order_by: function (n, old) {
      this.fetch_data();
    },
  },
  head() {
    return {
      title: "Markov Bot Archive/Search",
      meta: [
        // hid is used as unique identifier. Do not use `vmid` for it as it will not work
        {
          hid: "description",
          name: "description",
          content: "Page where you can find and search for messages sent by the Markov Bot during the 2022 JADS April Fool's Event",
        },
      ],
    };
  },
